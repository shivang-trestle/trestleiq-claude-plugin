    ...
    async handler(args, client) {
        try {
            ...
            if (data.phone_number)
                lines.push(`Phone: ${data.phone_number}`);
            ...
            if (typeof data.is_prepaid === 'boolean')
                lines.push(`Prepaid: ${data.is_prepaid ? 'yes' : 'no'}`);
            if (typeof data.activity_score === 'number')
                lines.push(`Activity score: ${data.activity_score}`);
            if (data.is_litigator_risk === true)
                lines.push(`Litigator risk: yes`);
            return {
